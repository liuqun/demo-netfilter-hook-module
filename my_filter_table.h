 ...
 * 返回值表示规则所在行序号.
 */
int insert_to_filter_table(struct filter_table *table, int where, const struct filter_comparator *fc, unsigned int code);

/**
 * 判定过滤表是否已满, 能否继续添加新规则.
 * 返回值为布尔量.
 */
int table_is_full(const struct filter_table *table);

/**
 * 清空整个过滤表.
 */
void flush_filter_table(struct filter_table *table);

/**
 * 修改过滤表的默认规则.
 * 输入值0表示丢弃DROP, 或1表示接受ACCEPT.
 */
void modify_filter_table_default_policy_code(struct filter_table *table, unsigned int default_policy_code);
